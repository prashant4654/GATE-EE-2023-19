...
\end{align}
The ROC for a given  $x\sbrak{n}$ , is defined as the range of  $z$ for which the Z-transform converges.\\
\begin{align}
\sum_{n=-\infty}^{\infty} |x\sbrak{n}z^{-n}| < \infty
\end{align}
The ROC cannot contain any poles.By definition a pole is a where  $X\brak{z}$ is infinite. Since  $X\brak{z}$ must be finite for all  $z$ for convergence, there cannot be a pole in the ROC\\
If  $x\sbrak{n}$ is a finite-duration sequence, then the ROC is the entire z-plane, except possibly  $z=0$ or $|z|=\infty$.\\
If  $x\sbrak{n}$ is a right-sided sequence, then the ROC extends outward from the outermost pole in  X\brak{z}. \\
If $x\sbrak{n}$ is a left-sided sequence, then the ROC extends inward from the innermost pole in  X\brak{z}.\\
If $x\sbrak{n}$ is a two-sided sequence, the ROC will be a ring in the z-plane that is bounded on the interior and exterior by a pole.\\
